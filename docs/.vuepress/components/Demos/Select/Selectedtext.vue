<template>
  <div class="con-select-example">
    <vs-select
      ...
      v-model="select1"
      >
      <vs-select-item v-model:is-selected="item.isSelected" :key="index" :modelValue="item.value" :text="item.isSelected?item.selectedText:item.label" v-for="item,index in options1" />
    </vs-select>
  </div>
</template>
